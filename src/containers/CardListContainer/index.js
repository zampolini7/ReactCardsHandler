...
import { getCards } from "../../redux/cardsRedux/actions";
...
const CardListContainer = () => {
  const userIdLogued = useSelector((state) => state.loginReducer.user);
  console.log(userIdLogued);

  const dispatch = useDispatch();

  const cards2 = useSelector((state) => state.cardReducer.cards);
  console.log(cards2);

  useEffect(() => {
    dispatch(getCards(userIdLogued));
  }, [userIdLogued || cards2]);
  ...
  return loading === false ? (
    ...
    <h2> Estamos cargando tus tarjetas</h2>
  );
};
