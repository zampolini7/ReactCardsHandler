...
export const saveCardsInFB = async (userUid, valueOfForm) => {
  const TYPE_OF_CARDS = {
    Mastercard:
  ...
  const creditCard = valueOfForm.creditCard;
  const nameOfCreditCompany = TYPE_OF_CARDS[creditCard] || DEFAULT_CARD;
  let img = nameOfCreditCompany;

  try {
    const docRef = await addDoc(collection(db, `users/${userUid}/cards`), {
      creditCard: creditCard,
      img: img || "",
      kindOfCreditCard: valueOfForm.kindOfCreditCard,
      name: valueOfForm.name,
      numberOfCreditCard: valueOfForm.numberOfCreditCard,
      until: valueOfForm.until,
    });
